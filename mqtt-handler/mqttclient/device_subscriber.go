...
func forwardToSpring(req *types.DeviceRegisterRequest, m *MQTTClient) {
	...
	if resp.StatusCode == http.StatusOK && springResp.Status == "OK" {
		log.Printf("[INFO] 기기 등록 성공 (deviceId=%d) at %s", req.DeviceId, springResp.Timestamp)
	} else {
		log.Printf("[WARN] 기기 등록 실패 (deviceId=%d) status=%s, timestamp=%s",
			req.DeviceId, springResp.Status, springResp.Timestamp)
	}
	...
	if token := m.mqttClient.Publish(ackTopic, 1, false, ackPayload); token.Wait() && token.Error() != nil {
		...
		log.Printf("[INFO] MQTT ack 발행 완료 (topic=%s, deviceId=%d)", ackTopic, req.DeviceId)
	}
}
